 ...
 *
 * Format: Name | Synonyme (Semikolon) | Formel | SMILES | CAS | PubChem-CID | Kategorie | Kurzbeschreibung
 *
 * Die Grundtabelle enthält geprüfte CAS-Nummern und PubChem-CIDs. Die
 * erweiterten Tabellen in ./substanceTables ergänzen weitere Stoffe; deren
 * Kennungen löst die App bei Bedarf online über PubChem auf.
 */
import { molarMass, parseFormula, toHillFormula } from '../chem/formula';
...
Hexan|n-Hexan|C6H14|CCCCCC|110-54-3|8058|Alkan|Unpolares Lösungsmittel, Hauptbestandteil von Petrolether
Phenolphthalein||C20H14O4|O=C1OC(c2ccc(O)cc2)(c2ccc(O)cc2)c2ccccc12|77-09-8|4764|Indikator|Säure-Base-Indikator, Umschlag bei pH 8,2–10
...
/** Erzeugt eine URL-taugliche Kennung aus dem Stoffnamen. */
export function substanceSlug(name: string): string {
  return name
    .toLowerCase()
    .replace(/ä/g, 'ae')
    .replace(/ö/g, 'oe')
...
function parseTable(table: string): Substance[] {
  return table
    .split('\n')
    .map((line) => line.trim())
    .filter(Boolean)
    .map((line) => {
      ...
      return {
        id: substanceSlug(name),
        name,
        synonyms: synonyms ? synonyms.split(';').map((entry) => entry.trim()).filter(Boolean) : [],
        formula,
...
function mergeUnique(...lists: Substance[][]): Substance[] {
  const seen = new Set<string>();
  const merged: Substance[] = [];
  for (const list of lists) {
    for (const substance of list) {
      if (seen.has(substance.id)) continue;
      seen.add(substance.id);
      merged.push(substance);
    }
  }
  return merged;
}
...
export function substanceById(id: string): Substance | undefined {
...
/** Summenformel in Hill-Notation als Vergleichsschlüssel; null, wenn nicht lesbar. */
export function formulaKey(formula: string): string | null {
  try {
...
/** Findet Stoffe mit gleicher Summenformel (Hill-Vergleich, Kristallwasser inklusive). */
export function substancesByFormula(formula: string): Substance[] {
  const target = formulaKey(formula);
  if (!target) return [];
  return SUBSTANCES.filter((substance) => formulaKey(substance.formula) === target);
}
